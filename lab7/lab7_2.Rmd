...
penguins %>% 
  group_by(island) %>% 
  summarise(mean_bill_lg=mean(bill_length_mm, na.rm = TRUE),
  mean_bill_depth=mean(bill_depth_mm, na.rm = TRUE),
  mean_flipper_lg=mean(flipper_length_mm, na.rm = TRUE),
...
penguins %>% 
  group_by(islands) %>% 
  summarise(number_NAs=sum(is.na(body_mass_g)),##couldnt figure out 
            n=n())
```
...
```{r}
penguins %>%
  summarize(across(contains("mm"), \(x) mean(x, na.rm = TRUE))) #use this to correct error
```
