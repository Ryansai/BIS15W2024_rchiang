...
```{r}
ggmap(map1) + geom_point(data = sharks_dups, aes(longitude, latitude, color=injury), size=0.5, alpha = 1)
```

4. What if you only wanted to map the fatalities?
```{r}

ggmap(map1) + geom_point(data = filter(sharks_dups, injury == "fatal"), aes(longitude, latitude), size=0.4)
```
