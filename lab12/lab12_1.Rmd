...
## you do not knit an install.packages so hashtag out
...
```{r}
homerange %>% 
  ggplot(aes(class, log10.mass, group=taxon)) + geom_boxplot()
```

4. Use `fill` to show the range of `log10.mass` by taxonomic class with color codes.
```{r}
homerange %>% 
  ggplot(aes(class, log10.mass, group=taxon, fill=taxon)) + geom_boxplot(alpha = .5)
```
...
deserts %>% 
  ...
  group_by(year, species_id) %>% 
  summarise(n=n(), .groups = 'keep') %>% 
  ggplot(aes(x=year,y=n, group= species_id, color=species_id)) + geom_line() + geom_point(shape=4) + #you can experiment with shapes
  theme(axis.text.x=element_text(angle = 60, hjust=1)) + labs(title="Number of Samples for Species DM & DS", x= "Year", y="n")
```
...
homerange %>% 
  ...
  geom_histogram(bins = 60)+ #we can adjust the number of bins with the bins argument
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_histogram(color = "black", fill = "wheat1", bins=10)+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ...
  geom_histogram(aes(y=after_stat(density)), fill = "violetred4", color= "black")+ geom_density(color="orange")
```
...
homerange %>% 
  ...
  ggplot(aes(x=mass_catagory, fill=trophic.guild))+ geom_bar(position = "dodge")
```
...
homerange %>% 
  mutate(range_catagory = case_when(log10.hra < 3.65 ~ "very small",
                                    ...
                                    log10.hra > 4.59 & log10.hra <= 6.02 ~ "medium", 
                                    log10.hra > 6.02 ~ "large")) %>% 
  ggplot(aes(x=range_catagory, fill=class)) + geom_bar(position = "dodge", alpha = .4, color= "black")
```
