...
```{r}
bovidae_big <- filter(mammals, family == "Bovidae" & mass >= 45000)
```
...
```{r}
monkey <- filter(mammals, genus == "Lophocebus"| genus == "Erythrocebus"| genus == "Macaca")
monkey_features <- select(monkey, mass, gestation, newborn)

```

```{r}
order(monkey_features$newborn)
monkey
```
### Macaca maura
