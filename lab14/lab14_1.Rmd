...
selectInput("x", "Select Y Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), selected = "bill_length_mm")
    plotOutput("plot", width="500px", height="400px")
)

server <- function(input, output) {
  output$plot <- renderplot({
    ggplot(data = penguins, aes_string(x=inputs$x, y=input$y, color="species")) + geom_point() + theme_light(bases_size=18) } 
}
...
server <- function(input,output, session) {output$plot <- renderPlot({
  ...
  ggplot(data=homerange,aes_string(x="locomotion", fill=input$x)) + geom_bar(position = "dodge", alpha = .8, color="black")
})}
