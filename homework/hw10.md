...
```r
deserts %>% 
  ggplot(aes(taxa)) + geom_bar() + scale_y_log10()
```
...
```r
deserts %>% 
  ggplot(aes(taxa, fill = plot_type)) + geom_bar() + scale_y_log10()
```
...
```r
deserts %>% 
  ggplot(aes(species, weight)) + geom_boxplot(na.rm = T) + coord_flip()
```
...
```r
deserts %>% 
  ggplot(aes(species, weight)) + geom_point(na.rm = T) + coord_flip()
```
...
```r
deserts %>% 
  ggplot(aes(weight, hindfoot_length)) + geom_point(na.rm = T, size= .5)
```
...
```r
deserts %>% 
  mutate(weightohindfoot = weight/hindfoot_length) %>%
  filter(species_id == "NL" | species_id == "DS") %>% 
  ggplot(aes(species_id, weightohindfoot, fill=sex)) + geom_boxplot(na.rm = T, alpha = .5)
```
...
deserts %>% 
  group_by(year) %>% 
  ggplot(aes(species, weight, fill = year)) + geom_col(na.rm = T) + coord_flip()
```
