...
---
###knit compleated exam open html save as pdf then upload as pdf
## Instructions
...
wolves %>% 
  group_by(park) %>% 
  summarise(names_parks = n_distinct(park))
...
wolves %>% 
  group_by(park) %>% 
  summarise(number_wolf_packs = n_distinct(pack)) %>% 
  arrange(desc(number_wolf_packs))
```
...
wolves %>% 
  group_by(park) %>% 
  summarise(most_human_caused_deaths = sum(mort_all, na.rm = TRUE)) %>% 
  arrange(desc(most_human_caused_deaths))
```
...
ynp %>% 
  filter(pack == "druid") %>% 
  summarize(average_pack_size_druid = mean(packsize_aug, na.rm = TRUE))
...
```
### Because there are no more data points after 2010 for the "druid' group I assume the group died off or the scientists stopped being able to record the size for the pack.
...
ynp %>% 
  group_by(pack) %>% 
  summarise(best_persistance = sum(persisty1)) %>% 
  arrange(desc(best_persistance))
```
### The Mollies have the best persistance out of all ynp packs after googling I found they prey on bison
...
wolves %>% 
  group_by(pack) %>% 
  summarise(avg_death_by_human = mean(mort_all)) %>% 
  arrange(desc(avg_death_by_human))
```
