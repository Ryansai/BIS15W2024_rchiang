...
```
### factor has discrete values/ can be repeated multiple times
### character is any string of number, factor is repeated string of number
```{r}
levels(hot_springs$scientist)
```
### levels tells us number of factor 
## Practice
...
```
### ^ this opens up the data, you need tha "data/" to tell you where it is, it will not open if not included
...
summary(fish)
```
### summary gives you basic stats
`glimpse()` is another useful summary function.
```{r}

```
### glimpse also tells you a summary, is good bc ledford does not trust stats given by summary
...
little_fish
```
### filter pulls out only certian data, <= is less than or equal to
## Practice
