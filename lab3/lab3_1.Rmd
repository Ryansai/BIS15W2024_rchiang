...
### It seems to take into consideration different types of data, not just numeric when making to data frame 
```{r}
...
### prof says all names should be in lower case, because upper and lower case are different variables
```{r}
...
hsprings_plus_depth <- data.frame(hsprings, depth_ft)
hsprings_plus_depth
```
### ^ should have used cbind to update hsprings instead of making new variable 
...
```{r}
write.csv(hsprings_plus_depth, "hsprings.csv", row.names = FALSE)
```
