...
### tidyverse uses commands that work together better. Tidyverse requires 'tidydata'. tidy data is data that flows together/ is organized.
...
```
### select(objectdataframe, "quotedvariable", "quotedvariable")
...
```{r}
fish_subset <- select(fish, "lakeid", "length")
```
...
summary(mammals)
```
### bc min weight is -999 we suspect they substituted that for NA
```{r}
...
```{r}
mammals_new <- rename(mammals, genus="Genus", wean_mass="wean mass", litters_per_year = "litters/year", litter_size = "litter size")
```
