...
life_history %>% 
  ...
  ggplot(aes(x = order,y = mean_mass)) + geom_col()+coord_flip()## you dont always need to enter x= or y= directly, could just (order, mean_mass)
```
...
life_history %>% 
  group_by(order) %>% 
  summarise(mean_mass=mean(mass, na.rm = T)) %>% 
  ggplot(aes(x = reorder(x = order,mean_mass), y=mean_mass)) + geom_col()+coord_flip() + scale_y_log10() + ## look back at his notes
```
...
```{r}
life_history %>% 
  ggplot(aes(gestation,wean_mass)) + geom_point() + geom_jitter(na.rm=T) + scale_y_log10()
```
...
```{r}
elephants %>% 
  ggplot(aes(age, height)) + geom_point() + geom_smooth(method = lm, se=F)
```
