...
```{r}
p + geom_point(aes(color=thermoregulation), size = 1.5) #for scatterplots, color is the same as fill in barplots, r does not see points as something that can be filled 
```
...
```{r}
homerange %>% 
  ggplot(aes(taxon)) + geom_bar() + coord_flip() + labs(title = "Observation by Taxon", x = "Taxonomic Group")
```
...
```{r}
homerange %>% 
  ggplot(aes(taxon, fill=trophic.guild)) + geom_bar() + coord_flip() + labs(title = "Observation by Taxon", x = "Taxonomic Group")
```

We can also have counts of each trophic guild within taxonomic group shown side-by-side by specifying `position="dodge"`.
```{r}
homerange %>% 
  ggplot(aes(taxon, fill=trophic.guild)) + geom_bar(position= "dodge") + coord_flip() + labs(title = "Observation by Taxon", x = "Taxonomic Group")
```
...
```{r}
penguins %>% 
  ggplot(aes(island, fill = species)) + geom_bar(position = "dodge") + labs(title = "Count of Penguins by Island", x = "Island")
```
...
penguins %>% 
  filter(sex != "NA") %>% 
  ggplot(aes(island, fill = sex)) + geom_bar(position = "dodge") + labs(title = "Count of Penguins by Island", x = "Island")
```
...
homerange %>% 
  ...
  geom_boxplot(alpha=0.4) #### alpha= controls color transparency
```
